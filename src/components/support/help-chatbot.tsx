...
export function HelpChatbot({ supportEmail }: { supportEmail: string }) {
  ...
  // A one-time nudge so first-time visitors notice the chat button exists —
  // dismissed permanently (this browser) once closed or once the chat is
  // actually opened.
  useEffect(() => {
  ...
  const draft = useRef<{ description?: string; category?: string; firstName?: string; lastName?: string; email?: string; phone?: string; ticketNumber?: string }>({});
  ...
  async function advance(text: string) {
    switch (stage) {
      case "greeting":
      case "idle": {
        if (/^track/i.test(text) || /^TKT-\d+/i.test(text)) {
          bot("Sure — what's your reference number? (e.g. TKT-000123)");
        ...
        // Treat anything else as the start of a complaint description directly.
      ...
      case "confirm_category": {
        const match = TICKET_CATEGORIES.find((c) => c.toLowerCase() === text.toLowerCase());
      ...
      case "collect_contact": {
        ...
        const result = await chatCreateTicketAction({
      ...
      case "track_number": {
        draft.current.ticketNumber = text;
      ...
      case "track_email": {
        ...
        setBusy(true);
        const result = await chatTrackTicketAction(draft.current.ticketNumber ?? "", text);
        setBusy(false);
        if (result.ok) {
          bot(
  ...
  function handleDescriptionCollected(description: string) {
    ...
    if (suggestion) {
      ...
      bot("Which category best fits?", [...TICKET_CATEGORIES]);
      setStage("confirm_category");
    }
  }
    ...
    <>
      ...
      {showInvite && !open && (
        <div className="fixed bottom-20 right-4 z-40 flex max-w-[calc(100vw-2rem)] items-start gap-2 rounded-lg border border-border bg-card p-3 shadow-lg sm:bottom-24 sm:right-6 sm:max-w-64">
          <p className="text-sm">
            Need help? You can chat with our virtual assistant anytime.
          </p>
