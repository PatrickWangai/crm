...
import { submitPublicSupportRequestAction, chatTrackTicketAction } from "@/app/help/actions";
...
export function PublicSupportForm({
  ...
  if (state.success !== prevSuccess) {
    ...
    if (state.success && state.tracking && state.ticketNumber) {
  ...
  useEffect(() => {
    ...
    if (saved.contactEmail) {
      chatTrackTicketAction(saved.ticketNumber, saved.contactEmail).then((res) => {
        if (res.ok) {
          ...
          >
            <div className="w-full max-w-sm rounded-lg bg-card p-6 text-center shadow-lg" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Your reference number is <span className="font-mono font-semibold text-foreground">{activeResult.ticketNumber}</span>. Keep it —
